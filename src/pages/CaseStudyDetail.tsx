...
const CaseStudyDetail = () => {
  ...
  const casePath = `/cases/${caseStudy.id}`;
  const caseUrl = `${siteConfig.siteUrl}/${lang}${casePath}`;
  const seoTitle = `${caseStudy.name[lang]} — ${caseStudy.category[lang]} | ${siteConfig.brandName}`;

  return (
    <div className="min-h-screen flex flex-col">
      <Seo
        title={seoTitle}
        description={caseStudy.desc[lang]}
        lang={lang}
        path={casePath}
        jsonLd={[
          {
            ...
            itemListElement: [
              {
                ...
                item: `${siteConfig.siteUrl}/${lang}`,
              },
              {
                "@type": "ListItem",
                position: 2,
                name: t.cases.title,
                item: `${siteConfig.siteUrl}/${lang}#cases`,
              },
              { "@type": "ListItem", position: 3, name: caseStudy.name[lang] },
            ],
          },
          {
            "@type": "CreativeWork",
            "@id": `${caseUrl}#case`,
            name: caseStudy.name[lang],
            description: caseStudy.desc[lang],
            inLanguage: lang,
            url: caseStudy.link,
      ...
      <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/95 backdrop-blur">
        <div className="container flex h-14 md:h-16 items-center justify-between px-4">
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="icon" onClick={() => navigate(`/${lang}`)} className="h-8 w-8">
              <ArrowLeft className="h-4 w-4" />
            </Button>
            <span className="text-base md:text-lg font-bold truncate">{siteConfig.brandName}</span>
          </div>
      ...
      <main className="flex-1">
        ...
        <section className="relative">
          <div className="w-full h-64 md:h-96 overflow-hidden">
            <img
              src={caseStudy.image}
              alt={`${caseStudy.name[lang]} — ${caseStudy.category[lang]}`}
              width={1200}
              height={630}
              decoding="async"
                ...
                <Card>
                  <CardContent className="pt-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {caseStudy.features[lang].map((feature, idx) => (
                ...
                <Card>
                  <CardContent className="pt-6 space-y-4">
                    <div>
                      ...
                      <p className="font-medium">{caseStudy.category[lang]}</p>
                    </div>
                    <div>
                      ...
                      <p className="font-medium">{caseStudy.duration[lang]}</p>
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground mb-1">
                        {lang === "ru" ? "Результат" : "Result"}
                      </p>
                      <p className="font-medium text-primary">{caseStudy.result[lang]}</p>
                    </div>
                  </CardContent>
                </Card>
              ...
              {prevCase ? (
                <Button variant="ghost" asChild>
                  <Link to={`/${lang}/cases/${prevCase.id}`} className="flex items-center gap-2">
                    <ArrowLeft className="h-4 w-4" />
                    <span className="hidden sm:inline">{prevCase.name[lang]}</span>
              ...
              {nextCase ? (
                <Button variant="ghost" asChild>
                  <Link to={`/${lang}/cases/${nextCase.id}`} className="flex items-center gap-2">
                    <span className="hidden sm:inline">{nextCase.name[lang]}</span>
